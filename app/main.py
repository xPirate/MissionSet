...
@app.post("/data/new")
def create_item(title: str = Form(...), description: str = Form(""), tags: str = Form("")):
    created_at = datetime.utcnow().isoformat()
    with engine.begin() as conn:
        res = conn.execute(
            text("INSERT INTO items (title, description, tags, created_at) VALUES (:t, :d, :g, :c)"),
            {"t": title, "d": description, "g": tags, "c": created_at}
        )
        item_id = res.lastrowid if hasattr(res, "lastrowid") else None
        row = conn.execute(text("SELECT id, title, description, tags, created_at FROM items WHERE id = last_insert_rowid()")).one()
        doc = {
            "title": row.title,
